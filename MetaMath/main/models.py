...
class Student(models.Model):
    student_id = models.CharField(
        ...
        validators=[
            ...
            RegexValidator(r'^\d{6}$', 'El ID debe tener exactamente 6 dígitos.')
        ]
    )
    ...
    def clean(self):
        # Prevent duplicate ID with faculty
        if Faculty.objects.filter(faculty_id=self.student_id).exists():
            raise ValidationError("El ID ya está en uso por un profesor.")

    def save(self, *args, **kwargs):
        self.student_id = str(self.student_id).zfill(6)
...
class Faculty(models.Model):
    faculty_id = models.CharField(
        ...
        validators=[
            ...
            RegexValidator(r'^\d{4}$', 'El ID debe tener exactamente 4 dígitos.')
        ]
    )
    name = models.CharField(max_length=100, null=False)
    email = models.EmailField(max_length=100, null=True, blank=True)
    ...
    def clean(self):
        # Prevent duplicate ID with student
        if Student.objects.filter(student_id=self.faculty_id).exists():
            raise ValidationError("El ID ya está en uso por un estudiante.")

    def save(self, *args, **kwargs):
        self.faculty_id = str(self.faculty_id).zfill(4)
        self.full_clean()  # Calls clean()
        super().save(*args, **kwargs)
