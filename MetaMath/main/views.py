    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Student.objects.filter(student_id=id, password=password).exists():
                ...
                error_messages.append('Credenciales invalidas, por favor verifica tu ID y contraseña')
        else:
            error_messages.append('Datos invalidos, ¿Estas seguro que has escrito todo correctamente?')
    else:
        form = LoginForm()
    ...
    if is_faculty_authorised(request, code):
        if request.method == 'POST':
            ...
            if form.is_valid():
                form.save()
                messages.success(
                    request, 'Anuncio agregado exitosamente.')
...
def deleteAnnouncement(request, code, id):
    if is_faculty_authorised(request, code):
        try:
            ...
            announcement.delete()
            messages.warning(request, 'Anuncio eliminado exitosamente.')
            return redirect('/faculty/' + str(code))
        except:
            return redirect('/faculty/' + str(code))
    else:
...
def updateAnnouncement(request, code, id):
    if is_faculty_authorised(request, code):
        try:
            ...
            if form.is_valid():
                form.save()
                messages.info(request, 'Anuncio actualizado exitosamente.')
                return redirect('/faculty/' + str(code))
        except:
            return redirect('/faculty/' + str(code))
...
def addCourseMaterial(request, code):
    if is_faculty_authorised(request, code):
        if request.method == 'POST':
            ...
            if form.is_valid():
                form.save()
                messages.success(request, 'Nuevo material del curso agregado.')
                return redirect('/faculty/' + str(code))
            else:
...
def deleteCourseMaterial(request, code, id):
    if is_faculty_authorised(request, code):
        ...
        messages.warning(request, 'Material del curso eliminado.')
        return redirect('/faculty/' + str(code))
    else:
        return redirect('std_login')
    ...
    if request.session.get('student_id'):
        ...
        if request.method == 'POST':
            if student.password == request.POST['oldPassword']:
                ...
                messages.error(
                    request, 'La contraseña es incorrecta. Por favor, intenta de nuevo.')
    ...
    if request.session.get('student_id'):
        ...
        if request.method == 'POST':
            if request.FILES['photo']:
                ...
                student.save()
                messages.success(request, 'Foto cambiada exitosamente.')
                return redirect('/profile/' + str(student.student_id))
            else:
                messages.error(
                    request, 'Por favor selecciona una foto.')
